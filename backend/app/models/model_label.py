...
class Label(Base):
    ...
    validations = relationship("LabelValidation", back_populates="label_record", cascade="all, delete-orphan")

    __table_args__ = (
        Index("idx_label_image_id", "image_id"),
    )
...
class LabelValidation(Base):
    ...
    validator_id = Column(Integer, ForeignKey("user.id"), nullable=False)
    label = Column(String, nullable=False)
