  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final comp = await ref
          .read(competitionCreateProvider.notifier)
          .createCompetition(
            name: _nameCtl.text,
            ...
            overview: _overviewCtl.text.trim().isEmpty
                ? null
                : _overviewCtl.text.trim(),
            termsConditions: _termsCtl.text.trim().isEmpty
                ? null
                : _termsCtl.text.trim(),
            dataMarkdown: _dataMdCtl.text.trim().isEmpty
                ? null
                : _dataMdCtl.text.trim(),
            dataFormat: _selectedFormats.isEmpty
                ? null
                : _selectedFormats.toList(),
            dataExample: _dataExCtl.text.trim().isEmpty
                ? null
                : _dataExCtl.text.trim(),
            evaluation: _selectedEvaluation,
            scoringExample: _scoringExCtl.text.trim().isEmpty
                ? null
                : _scoringExCtl.text.trim(),
                ...
                .map(
                  (ext) => FilterChip(
                    ...
                    onSelected: (sel) => setState(() {
                      if (sel) {
                        _selectedFormats.add(ext);
                      } else {
                        _selectedFormats.remove(ext);
                      }
                    }),
                  ),
                )
            ...
            children: ['F1 score', 'Accuracy', 'Precision', 'Recall', 'ROC AUC']
                .map(
                  (m) => FilterChip(
                    ...
                    onSelected: (sel) =>
                        setState(() => _selectedEvaluation = sel ? m : null),
                  ),
                )
                .toList(),
          ),
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
              ...
              helperText:
                  'Similarity score 0–1 above which images are flagged.',
            ),
            validator: (v) {
              if (v == null || v.trim().isEmpty) return null;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.md),
          Text('Allowed model formats', style: theme.textTheme.labelLarge),
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: Text(
              'Add Team with ${_pendingEmails.length} member${_pendingEmails.length == 1 ? '' : 's'}',
            ),
          ),
